...
class Person(models.Model):
    ...
    visit_password = models.CharField('Пароль для просмотра', max_length=10, blank=True)
    ...
    def __str__(self):
        return f'{self.name} Создан: {timezone.localtime(self.date).strftime("%Y-%m-%d %H:%M:%S")} '

    def save(self, *args, **kwargs):
        self.slug = f'{self.slug}date{ slugify(timezone.localtime(self.date).strftime("%Y-%m-%d %H:%M:%S"))}'
